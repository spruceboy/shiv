...
class RackWelder
  ...
  def send_file_full(req_path, request, response, mime_type = 'image/png', _header_only = false)
    send_file_xsendfile(request, response, req_path, mime_type)
  end
  ...
  #  FIXME!!!
  #
  def send_file_full_rack(req_path, request, response, type = 'png', _header_only = false)
    ...
    same_response = case
                    when modified_since && !last_response_time = begin
                                                                   Time.httpdate(modified_since)
                                                                 rescue
                                                                   nil
                                                                 end then false
