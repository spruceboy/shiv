...
# not currently used - it was too resource intensive to manage.
...
class HeatMapper
  ...
  def initialize(cfg, logger)
    ...
    @side = 2**@z_level_map
    @grid = File.open(@cfg['stat_save_file']) { |x| YAML.load(x) } if File.exist?(@cfg['stat_save_file'])
    CalculateGrid() unless @grid
    StartStatWriter()
  end
...
class Imlib2HeatMapper < HeatMapper
  ...
  def GetImage
    ...
    im.delete!
    @cfg['stat_image_file'] + '.tmp.png'
  end

  def GetImageWithBackground
    GetImageWithBackgroundCmdLine()
  end
