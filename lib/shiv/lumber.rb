...
class LumberNoFile
  ...
  def msginfo(s)
    return unless @verbose
    format_for_output(@info_fd, 'INFO', s)
    format_for_output(STDOUT, 'INFO', s) unless @quiet
  end
  ...
  def msgdebug(s)
    return unless @debug
    format_for_output(@info_fd, 'DEBUG', s)
    format_for_output(STDOUT, 'DEBUG', s) unless @quiet
  end
  ...
  def log_xfer(request, _response, size, tm)
