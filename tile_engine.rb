...
class TileEngine
  ...
  def check_and_wait(x, y, z)
    ...
    while  !File.exist?(path) || File.size?(path) == 0
      ...
      counter -= 1
      fail "timed out while waiting for waiting on #{@cfg['title']}(#{x},#{y},#{z})" if counter < 0
      sleep(WAIT_TIME)
    end
  end
...
class ExternalTileEngine  < TileEngine
  ...
  def initialize(cfg, logger)
    ...
    @@idler = Idler.new(1, cfg['idler']) unless  @@idler    # Only create an idler if a instance is instaicated. My spelling sucks.  So does my coding.
  ...
  def alt_tile?(_x, _y, _z, request)
    return false unless @cfg['notice']
    return false unless @cfg['notice']['auth']
    return true if ENV['AUTH_TEST']
    return false unless request.env[@cfg['notice']['auth']]
  ...
  #
  def warning_text(img, _font, msg, x, y)
  ...
  def tile_gen(x, y, z)
    ...
    (-2).upto(2) { |dx| (-2).upto(2) { |dy| @@idler.add(self, x + dx * @cfg['tiles']['x_count'], y + dy * @cfg['tiles']['y_count'], z) unless dy == 0 && dx == 0 } }
